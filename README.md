...
Recall is an [Obsidian](https://obsidian.md) plugin that uses the [Readwise](https://readwise.io) CLI to run vector/semantic search over your highlights and saved articles. When you have a note open (or text selected), Recall shows related highlights and documents in a sidebar so you can pull quotes and ideas into your vault without leaving the editor.

## Features

- **Sidebar view** — Open a “Recall” pane that lists highlights and/or documents related to the active note.
...
On macOS, if the CLI is installed via Homebrew (e.g. under `/opt/homebrew/bin` or `/usr/local/bin`), Recall will try those locations when the path is left as `readwise`.
...
3. Search for **Recall**, install it, then enable it.
4. Open **Settings → Recall** and set the **Readwise CLI path** if it’s not `readwise` on your PATH.

### Manual install

1. Download the latest `main.js`, `manifest.json`, and `styles.css` from the releases (or build from source).
2. Copy them into `VaultFolder/.obsidian/plugins/obsidian-recall/`.
3. Reload Obsidian and enable **Recall** under Community plugins.

## Usage

- **Open the sidebar** — Click the book icon in the left ribbon, or run the command **Open Recall sidebar**. The pane opens on the right and runs a search for the current note.
- **Search for current note** — Command: **Search Readwise for current note**. Uses the active note’s content to run a semantic search (no need to have the Recall pane open first).
- **Search for selection** — Select text, then either run **Search Readwise for selection** from the command palette or use the editor context menu: **Search Readwise for selection**. Opens the Recall pane and searches using only the selected text.
- **Refresh** — In the Recall pane, use the **Refresh** button to run the search again with the same context.

### How the “current note” search term is built

When you switch notes or run “Search Readwise for current note”, Recall builds a search string from:
...
```bash
git clone https://github.com/your-username/obsidian-recall.git
cd obsidian-recall
npm install
npm run build
```

Then copy `main.js`, `manifest.json`, and `styles.css` (if present) from the project root into your vault’s `.obsidian/plugins/obsidian-recall/` folder.
